...
@with_metadata(schemas.ToolMetadata(
    ...
    requires=[],
    optional=['poscar_path', 'mlps', 'fmax', 'max_steps'],
    defaults={
        'poscar_path': f'{os.environ.get("MASGENT_SESSION_RUNS_DIR")}/POSCAR',
        'mlps': 'SevenNet',
        'fmax': 1.0,
...
def fast_simulation_using_mlps(
    poscar_path: str = f'{os.environ.get("MASGENT_SESSION_RUNS_DIR")}/POSCAR',
    mlps_type: Literal['SevenNet'] = 'SevenNet',
    fmax: float = 1.0,
    max_steps: int = 1000,
    ...
    try:
        ...
        if task_type == 'single_point':
            ...
            atoms = read(poscar_path, format='vasp')
            atoms.calc = calc
            opt = LBFGS(FrechetCellFilter(atoms), logfile=f'{task_dir}/masgent_mlps.log')
            ...
            return {
                ...
                'simulation_log_path': f'{task_dir}/masgent_mlps.log',
            ...
            os.makedirs(task_dir, exist_ok=True)
            structure = Structure.from_file(poscar_path)
            scale_factors = [0.85, 0.90, 0.95, 1.00, 1.05, 1.10, 1.15]
            volumes, energies = [], []
            for scale in scale_factors:
            ...
            return {
                'status': 'error',
                'message': f'Invalid task type: {task}. Choose either "single_point" or "eos".'
            }
